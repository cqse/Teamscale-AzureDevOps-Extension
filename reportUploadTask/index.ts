...
const pullRequestRevision = task.getVariable(
...
async function run() {
  try {
    ...
    task.setResult(
      ...
      `Upload to Teamscale failed with error: ${e.message}`
    );
  }
}

async function convertCoverageFiles(coverageFiles: string[]): Promise<string> {
  const outputXmlFile = path.join(
    path.dirname(coverageFiles[0]),
    "coverage.xml"
  );
  ...
  const codeCoverageRunner: toolRunner.ToolRunner = task.tool(
    codeCoverageExePath
  );
...
async function runUnsafe() {
  ...
  const uploadUrl = utils.createUploadUrl(
    teamscaleUrl,
    project,
    format,
    partition,
  ...
  if (coverageFiles.length > 0) {
    ...
    filesToUpload = filesToUpload
      .filter((file) => !utils.isCoverageFile(file))
      .concat(convertedCoverageFile);
    task.debug(`Now uploading ${filesToUpload}`);
  }

  const curlRunner: toolRunner.ToolRunner = createCurlRunner(
    username,
    accessKey,
    filesToUpload,
    uploadUrl
...
process.on("unhandledRejection", (error: Error) => {
  task.error(
    `Task failed. Please check the log for further details.\n${error.stack}`
  );
  task.setResult(
    task.TaskResult.Failed,
    `Upload to Teamscale failed with error: ${error.message}`
  );
});
